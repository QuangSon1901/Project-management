...
<div class="order_info-payment">
    <div class="order_place" style="border:none">
        <div class="order_place-heading">{{$ticketItem['nameFrom']}} - {{$ticketItem['nameTo']}}
        </div>
        <ol class="order_place-desc">
            <li>Ngày {{date('d', strtotime($ticketItem['ticket']->flight_time_departure))}} tháng {{date('m', strtotime($ticketItem['ticket']->flight_time_departure))}}</li>
            <li>-</li>
            <li>1 khách</li>
            <li>-</li>
            <li>{{$ticketItem['ticket']->ticket_class_name}}</li>
        </ol>
    </div>
    ...
    <div class="class_seat-desc-info-body">
        <div class="seat_desc-info-time">
            <div class="seat_desc-info-time-start">
                <h4>{{date('H:i', strtotime($ticketItem['ticket']->flight_time_departure))}}</h4>
                <h4 class="seat_desc-info-time-date">
                    {{date('d/m/Y', strtotime($ticketItem['ticket']->flight_time_departure))}}
                </h4>
            </div>
            <div class="seat_desc-info-time-dep">
                @if(date('H', strtotime($ticketItem['ticket']->flight_time_fly)) != 0)
                {{date('H', strtotime($ticketItem['ticket']->flight_time_fly))}}h
                @endif
                {{date('i', strtotime($ticketItem['ticket']->flight_time_fly))}}'
            </div>
            <div class="seat_desc-info-time-end">
                <h4>{{date('H:i', $ticketItem['timeArr'])}}</h4>
                <h4 class="seat_desc-info-time-date">
                    {{date('d/m/Y', $ticketItem['timeArr'])}}
                </h4>
            </div>
        </div>
        ...
        <div class="seat_desc-info-place">
            <div class="seat_desc-info-place-start">
                <h4>{{$ticketItem['nameFrom']}}</h4>
                <h4 class="seat_desc-info-place-airplane">
                    {{$ticketItem['aliasFrom']->airport_name}}
                </h4>
            </div>
            ...
            <div class="seat_desc-info-place-seat">
                ...
                <h4>{{$ticketItem['ticket']->ticket_class_name}}</h4>
                <h4 class="class_seat-it">EB</h4>
            </div>
            <div class="seat_desc-info-place-end">
                <h4>{{$ticketItem['nameTo']}}</h4>
                <h4 class="seat_desc-info-place-airplane">
                    {{$ticketItem['aliasTo']->airport_name}}
                </h4>
            </div>
        </div>
    </div>
