<footer class="footer">
    <div class="grid">
        <div class="footer-nav">
            <div class="footer-menu">
                <div class="footer-menu__list">
                    <a href="#" class="footer-menu__item">
                        Giới thiệu
                        <span class="underline footer-menu__item--underline"></span>
                    </a>
                    <a href="#" class="footer-menu__item">
                        Liên hệ
                        <span class="underline footer-menu__item--underline"></span>
                    </a>
                    <a href="#" class="footer-menu__item">
                        Video
                        <span class="underline footer-menu__item--underline"></span>
                    </a>
                    <a href="#" class="footer-menu__item">
                        Tải về
                        <span class="underline footer-menu__item--underline"></span>
                    </a>
                    <a href="#" class="footer-menu__item">
                        Tuyển dụng
                        <span class="underline footer-menu__item--underline"></span>
                    </a>
                    <a href="#" class="footer-menu__item">
                        Hình ảnh
                        <span class="underline footer-menu__item--underline"></span>
                    </a>
                    <a href="#" class="footer-menu__item">
                        Đến trang công ty
                        <span class="underline footer-menu__item--underline"></span>
                    </a>
                </div>
                <h5 class="footer-menu__copy-right">
                    © 2022 QSC CITY - Developed by TAB
                </h5>
            </div>
            <div class="footer-social">
                <div class="footer-social__list">
                    <a href="" class="footer-social__item">
                        <i class="footer-social__icon fab fa-youtube"></i>
                    </a>
                    <a href="" class="footer-social__item">
                        <i class="footer-social__icon fab fa-facebook-f"></i>
                    </a>
                    <a href="" class="footer-social__item">
                        <i class="footer-social__icon fab fa-google-plus-g"></i>
                    </a>
                    <a href="" class="footer-social__item">
                        <i class="footer-social__icon fab fa-twitter"></i>
                    </a>
                    <a href="" class="footer-social__item">
                        <i class="footer-social__icon fab fa-instagram"></i>
                    </a>
                </div>
            </div>
        </div>
    </div>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery.pjax/2.0.1/jquery.pjax.min.js" integrity="sha512-7G7ueVi8m7Ldo2APeWMCoGjs4EjXDhJ20DrPglDQqy8fnxsFQZeJNtuQlTT0xoBQJzWRFp4+ikyMdzDOcW36kQ==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdn.jsdelivr.net/npm/flatpickr"></script>
    <script src="<?php echo e(asset('user_asset/assets/js/search/search-js.js')); ?>"></script>
    <script src="<?php echo e(asset('user_asset/assets/js/login-modal/login-modal.js')); ?>"></script>
    
</footer><?php /**PATH C:\Users\Son\Desktop\Project-management\Backend\AirBooking\resources\views/user_page/footer.blade.php ENDPATH**/ ?>